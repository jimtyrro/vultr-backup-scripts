...
# This modification allows the user to set a custom snapshot limit for each instance interactively, providing more flexibility and control over the snapshot creation process.
...
# Add function to prompt for user input:
prompt_for_input() {
    local prompt=$1
...
# Prompt for variables before loading from .env
VULTR_API_KEY=$(prompt_for_input "Enter your Vultr API Key" "")
VULTR_DEFAULT_SNAPSHOT_LIMIT=$(prompt_for_input "Enter the default snapshot limit" "4")

# Save variables to .env file
cat > .env << EOF
VULTR_API_KEY=$VULTR_API_KEY
VULTR_DEFAULT_SNAPSHOT_LIMIT=$VULTR_DEFAULT_SNAPSHOT_LIMIT
EOF
...
if [ -f .env ]; then
    ...
    echo "No .env file found. Using entered values."
fi
...
for INSTANCE_ID in $instance_ids; do
    ...
    # Prompt user for snapshot limit
    echo "Enter snapshot limit for instance $INSTANCE_ID (press Enter for default: $DEFAULT_SNAPSHOT_LIMIT):"
    read USER_SNAPSHOT_LIMIT
    SNAPSHOT_LIMIT=${USER_SNAPSHOT_LIMIT:-$DEFAULT_SNAPSHOT_LIMIT}
