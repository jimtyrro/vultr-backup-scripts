#!/bin/sh
# This improved version incorporates the suggested enhancements, including better error handling, use of environment variables for sensitive information, JSON parsing with jq, input validation, and a dry-run option. By using this method, you keep your sensitive information separate from the script, making it easier to manage and more secure. Remember to add .env to your .gitignore file if you're using version control to prevent accidentally committing sensitive information.
...
INSTANCE_ID=${VULTR_INSTANCE_ID:?"VULTR_INSTANCE_ID must be set"}
SNAPSHOT_LIMIT=${VULTR_SNAPSHOT_LIMIT:-4}
...
if [ -z "$instance_details" ]; then
	echo "Error: Failed to retrieve instance details. Check if INSTANCE_ID is valid."
	exit 1
fi
...
# Get snapshots
snapshots=$(vultr_api_call GET "snapshots")
